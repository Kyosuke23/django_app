...
class ProductCategoryModelTest(TestCase):
    def test_str_returns_name(self):
        '''__str__ がカテゴリ名を返す'''
        cat = ProductCategory.objects.create(product_category_nm='家電')
        self.assertEqual(str(cat), '家電')

    def test_unique_constraint_on_name(self):
        '''カテゴリ名がユニーク制約に違反すると IntegrityError'''
        ProductCategory.objects.create(product_category_nm='食品')
...
class ProductModelTest(TestCase):
    ...
    def test_product_cd_validation(self):
        '''product_cd が正規表現違反なら ValidationError'''
        product = Product(
            product_cd='INVALID$',
            product_nm='不正コード',
            start_date=date(2025, 1, 1),
    ...
    def test_unique_constraint_product_cd_period(self):
        '''(product_cd, start_date, end_date) の複合ユニーク制約'''
