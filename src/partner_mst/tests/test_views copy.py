...
class PartnerViewTests(TestCase):
    ...
    def test_6_2_1(self):
        ...
        self.assertEqual(f"期待: {list(HEADER_MAP.keys())}, 実際: ['partner_name_test', 'partner_name_kana', 'partner_type', 'contact_name', 'email', 'tel_number', 'postal_code', 'state', 'city', 'address', 'address2']", res_json['details'])
        self.assertEqual(Partner.objects.count(), 0)
    ...
    def _assert_csv_matches_queryset(self, rows, queryset):
        ...
        for row, partner in zip(rows[1:], queryset):
            expected = [
                ...
                for col in list(HEADER_MAP.keys())
            ]
